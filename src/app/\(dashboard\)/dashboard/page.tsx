  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        />
        <div className={`p-4 rounded-xl border border-zinc-200/80 dark:border-zinc-700/80 ${healthBg} flex flex-col justify-between`}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">Pipeline Health</span>
      ...
      <div className="rounded-xl border border-zinc-200/80 dark:border-zinc-700/80 bg-white dark:bg-zinc-900/50 p-5">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Funnel Conversion</h2>
            <p className="text-xs text-zinc-500 mt-0.5">Leads moving through active pipeline</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p className="text-lg font-bold text-zinc-900 dark:text-zinc-100 tabular-nums">{data.activePipeline}</p>
              <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-wider">Active</p>
            </div>
            <div className="h-8 w-px bg-zinc-200 dark:bg-zinc-800" />
            <div className="text-right">
              <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400 tabular-nums">{data.closedWon}</p>
              <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-wider">Closed</p>
            </div>
          </div>
        </div>

        <div className="space-y-4">
          {PIPELINE_STAGES.filter(s => !['closed_lost', 'no_response'].includes(s)).map((stage) => {
            ...
            return (
              <div key={stage} className="space-y-1.5">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="font-medium text-zinc-700 dark:text-zinc-300">{STAGE_LABELS[stage]}</span>
                  <span className="font-bold text-zinc-900 dark:text-zinc-100 tabular-nums">{count}</span>
                </div>
                <div className="h-2 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                  <div
                    className={`h-full ${stageBarColors[stage]} transition-all duration-500`}
        ...
        <div className="mt-6 flex items-center justify-center">
          <Link href="/pipeline" className="text-xs font-semibold text-zinc-500 hover:text-red-600 transition-colors flex items-center gap-1">
            Open full pipeline board <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MiniKPI icon={<Clock className="h-3.5 w-3.5 text-blue-500" />} label="Avg Days to Reply" value={`${data.avgDaysToReply}d`} />
        <MiniKPI icon={<TrendingUp className="h-3.5 w-3.5 text-amber-500" />} label="Avg Touchpoints" value={data.avgTouchpoints.toString()} />
