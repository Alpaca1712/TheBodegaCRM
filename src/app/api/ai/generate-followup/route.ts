...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await EmailService.generateFollowup(
      validation.data.lead as unknown as Lead,
      validation.data.emailThread as Array<Record<string, unknown>>,
      validation.data.followUpNumber,
      {
        customContext: validation.data.customContext,
        memories: memories as Array<{ memory_type: string; content: string }>
      }
    )
