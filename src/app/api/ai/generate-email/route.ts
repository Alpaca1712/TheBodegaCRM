...
export async function POST(request: NextRequest) {
  try {
    ...
    const [mckenna, hormozi] = await Promise.all([
      EmailService.generateInitial(lead, 'mckenna', { customContext, memories }),
      EmailService.generateInitial(lead, 'hormozi', { customContext, memories }),
    ])

    return NextResponse.json({ mckenna, hormozi })
  } catch (error) {
