  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{investor.name}</h1>
          {investor.firm && <p className="text-lg text-slate-500 mt-1">{investor.firm}</p>}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {investor.phone && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700">{investor.phone}</span>
                </div>
              )}
              {investor.website && (
                <div className="flex items-center gap-2">
            ...
            {investor.thesis && (
              <div className="mt-6 pt-4 border-t border-slate-200">
                <h3 className="text-sm font-medium text-slate-700 mb-2">Investment Thesis</h3>
                <p className="text-slate-600">{investor.thesis}</p>
              </div>
            )}
            ...
            {investor.notes && (
              <div className="mt-4 pt-4 border-t border-slate-200">
                ...
                <p className="text-slate-600 whitespace-pre-wrap">{investor.notes}</p>
              </div>
            )}
          </div>
          ...
          {/* Investments */}
          <div className="bg-white rounded-xl shadow p-6">
            <h2 className="text-lg font-semibold text-slate-900 mb-4">Investments</h2>
            {investments.length === 0 ? (
              <p className="text-slate-500 text-sm">No investments recorded yet.</p>
            ) : (
              <div className="space-y-3">
                {investments.map((inv) => (
                  <div key={inv.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                    <div>
                      <p className="font-medium text-slate-900">{inv.round_name}</p>
                      <p className="text-sm text-slate-500">{inv.instrument} {inv.pitch_date ? `· Pitched ${new Date(inv.pitch_date).toLocaleDateString()}` : ''}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${STAGE_COLORS[inv.stage] || 'bg-slate-100 text-slate-600'}`}>
                        {inv.stage.replace('_', ' ')}
                      </span>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <dl className="space-y-3">
              ...
              {(investor.check_size_min || investor.check_size_max) && (
                <div className="flex justify-between">
                  <dt className="text-sm text-slate-500">Check Size</dt>
              ...
              <div className="flex justify-between">
                ...
                <dd className="text-sm text-slate-700">{new Date(investor.created_at).toLocaleDateString()}</dd>
              </div>
            </dl>
          </div>
        </div>
