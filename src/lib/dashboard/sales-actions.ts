...
export function getLeadBestAction({
  ...
  const potentialActions: SalesAction[] = []
  ...
  return potentialActions.reduce((best, current) =>
...
export function buildSalesActionPlan({
  ...
  const emailsByLead = new Map<string, { inbound: ActionEmail[], outbound: ActionEmail[] }>()

  for (const email of inboundEmails) {
    if (!emailsByLead.has(email.lead_id)) emailsByLead.set(email.lead_id, { inbound: [], outbound: [] })
    emailsByLead.get(email.lead_id)!.inbound.push(email)
  }

  for (const email of outboundEmails) {
    if (!emailsByLead.has(email.lead_id)) emailsByLead.set(email.lead_id, { inbound: [], outbound: [] })
    emailsByLead.get(email.lead_id)!.outbound.push(email)
  }

  const actions: SalesAction[] = []

  for (const lead of leads) {
    const leadEmails = emailsByLead.get(lead.id) || { inbound: [], outbound: [] }
    const action = getLeadBestAction({
      lead,
      inboundEmails: leadEmails.inbound,
      outboundEmails: leadEmails.outbound,
      now
    })
