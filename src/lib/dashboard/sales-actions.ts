...
export function getLeadBestAction(
  ...
  const actions = getLeadActions(lead, latestInboundAt, latestOutboundAt, outboundCount, now)
...
export function buildSalesActionPlan({
  ...
  for (const lead of leads) {
    ...
    const bestAction = getLeadBestAction(lead, latestInboundAt, latestOutboundAt, outboundCount, now)
    if (bestAction) {
...
export function latestEmailByLead(emails: ActionEmail[]) {
...
export function emailCountByLead(emails: ActionEmail[]) {
...
export function mostRecentDate(values: Array<string | Date | null | undefined>) {
