...
export function getLeadBestAction(
  ...
  const latestInboundAt = mostRecentDate([
    ...
    ...inboundEmails.map((e) => e.created_at),
  ])
  ...
  const candidateActions: SalesAction[] = []
  ...
  // 4. Investor Memo (Across multiple stages)
  if (lead.type === 'investor' && !lead.investor_memo) {
    const isMeetingStage = ['meeting_booked', 'meeting_held'].includes(lead.stage)
    const isOutreachStage = ['researched', 'email_sent', 'follow_up', 'no_response'].includes(lead.stage)

    if (isMeetingStage || isOutreachStage) {
      candidateActions.push({
        ...
        leadStage: lead.stage,
        companyName: lead.company_name,
        priority: isMeetingStage || icp >= 80 ? 'high' : 'medium',
        category: 'investor_memo',
        title: isMeetingStage ? `Draft memo for ${lead.contact_name}` : `Generate memo for ${lead.contact_name}`,
        reason: isMeetingStage
          ? `${lead.company_name} meeting is ${lead.stage === 'meeting_booked' ? 'booked' : 'complete'} and needs a tailored investor memo.`
          : `${lead.contact_name} is an investor lead missing a personalized memo.`,
        recommendedAction: isMeetingStage
          ? 'Generate the investor memo while the conversation context is fresh.'
          : 'Create an Amazon-style one-pager to drop in the next follow-up.',
        ctaLabel: 'Generate memo',
        ctaHref: `/leads/${lead.id}`,
        score: (isMeetingStage ? 880 : 840) + icp + (isMeetingStage ? recencyBoost(daysSinceInbound ?? daysSinceOutbound) : 0),
      })
    }
  }
  ...
  return candidateActions.sort((a, b) => b.score - a.score)[0]
}
...
export function buildSalesActionPlan({
  ...
  const outboundByLead = groupEmailsByLead(outboundEmails)
  const inboundByLead = groupEmailsByLead(inboundEmails)
  const actions: SalesAction[] = []

  for (const lead of leads) {
    const bestAction = getLeadBestAction(
      lead,
      outboundByLead.get(lead.id) ?? [],
      inboundByLead.get(lead.id) ?? [],
      now
    )
    if (bestAction) {
