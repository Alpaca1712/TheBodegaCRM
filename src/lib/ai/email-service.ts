...
import type { Lead, BattleCard } from '@/types/leads'
...
function buildInitialUserPrompt(
  ...
  const bc = lead.battle_card as BattleCard | null;
...
function buildFollowupUserPrompt(input: {
  lead: Lead,
...
INSTRUCTIONS: Extract the CORE STRATEGY or OFFER from Daniel's notes and build the email around it.`)
  }

  const context = sections.join('\n\n')

  if (lead.stage === 'replied') {
    return `${context}\n\n=== TASK: THEY REPLIED ===\nACA framework. Match length. Value next step.`
  }

  if (lead.stage === 'meeting_held') {
    return `${context}\n\n=== TASK: POST-MEETING FOLLOW-UP ===\nWarm continuation. 60-100 words.`
  }

  if (followUpNumber === 1) {
    return `${context}\n\n=== TASK: FOLLOW-UP #1 (Day 4 Bump) ===\nShort. No reference to original. New value.`
  }

  if (followUpNumber === 2) {
    return `${context}\n\n=== TASK: FOLLOW-UP #2 (Day 9 Value Drop) ===\nLead with deliverable. SPECIFIC.`
  }

  if (followUpNumber === 3) {
    return `${context}\n\n=== TASK: FOLLOW-UP #3 (Day 14 Channel Switch) ===\nLinkedIn/Twitter DM. Casual.`
  }

  return `${context}\n\n=== TASK: BREAK-UP (Day 21+) ===\nStanding offer. Memorable exit.`
}
...
export async function generateInitialEmail(
  ...
  const [mckennaRaw, hormoziRaw] = await Promise.all([
  ...
  const mckenna = {
    subject: stripEmDashes(mckennaRaw.subject),
    body: stripEmDashes(mckennaRaw.body),
    ctaType: 'mckenna' as const,
    wordCount: countWords(mckennaRaw.body),
    quality: checkEmailQuality(mckennaRaw.subject, mckennaRaw.body, 'initial'),
  }

  const hormozi = {
    subject: stripEmDashes(hormoziRaw.subject),
    body: stripEmDashes(hormoziRaw.body),
    ctaType: 'hormozi' as const,
    wordCount: countWords(hormoziRaw.body),
    quality: checkEmailQuality(hormoziRaw.subject, hormoziRaw.body, 'initial'),
  }

  return { mckenna, hormozi }
}
...
export async function generateFollowupEmail(params: {
  ...
  const { lead, emailThread, followUpNumber, ctaStyle, customContext, memories } = params
  ...
  const result = await generateJSON<{
    ...
    buildFollowupUserPrompt({ lead, emailThread, followUpNumber, customContext, memories }),
    { temperature: 0.95, maxTokens: 4096 }
  )

  return {
    subject: stripEmDashes(result.subject),
    body: stripEmDashes(result.body),
    ctaType: ctaStyle,
    wordCount: countWords(result.body),
    quality: checkEmailQuality(result.subject, result.body, 'follow_up'),
  }
}
