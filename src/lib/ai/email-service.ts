...
LANGUAGE: Write like you're texting a smart friend. No jargon. No "agentic pentesting," "adversarial inputs," "prompt injection," "jailbreaking," "data exfiltration," or "attack surface" unless the lead uses those terms first.
...
export async function generateInitialEmail(
  ...
  const buildUserPrompt = (ctaStyle: 'mckenna' | 'hormozi') => {
...
${research}${memorySection}${customSection}`
  ...
  const [mckennaResult, hormoziResult] = await Promise.all([
    generateJSON<{ subject: string; body: string }>(
      systemPrompt,
      buildUserPrompt('mckenna'),
      { temperature: 0.95, maxTokens: 4096 }
    ),
    generateJSON<{ subject: string; body: string }>(
      systemPrompt,
      buildUserPrompt('hormozi'),
      { temperature: 0.95, maxTokens: 4096 }
    ),
  ])

  const mckennaBody = stripEmDashes(mckennaResult.body)
  const mckennaSubject = stripEmDashes(mckennaResult.subject)
  const hormoziBody = stripEmDashes(hormoziResult.body)
  const hormoziSubject = stripEmDashes(hormoziResult.subject)

  return {
    mckenna: {
      subject: mckennaSubject,
      body: mckennaBody,
      ctaType: 'mckenna',
      wordCount: countWords(mckennaBody),
      quality: checkEmailQuality(mckennaSubject, mckennaBody, 'initial'),
    },
    hormozi: {
      subject: hormoziSubject,
      body: hormoziBody,
      ctaType: 'hormozi',
      wordCount: countWords(hormoziBody),
      quality: checkEmailQuality(hormoziSubject, hormoziBody, 'initial'),
    },
  }
}
...
${hasStrategy ? 'Length: as long as the strategy needs, but tight.' : 'MAX: 40-60 words.'}`
    ...
    taskPrompt = `=== TASK: POST-MEETING FOLLOW-UP ===
...
- The follow-up should make them feel like working with you is already underway, not like they still need to decide.`
    ...
    taskPrompt = `=== TASK: FOLLOW-UP #1 (Day 4, The Bump + New Insight) ===
...
- The reader should learn something new or get offered something useful. No empty bumps.`
    ...
    taskPrompt = `=== TASK: FOLLOW-UP #2 (Day 9, The Value Drop) ===
...
- Slightly funny or clever framing. Not corporate.`
    ...
    taskPrompt = `=== TASK: FOLLOW-UP #3 (Day 14, Channel Switch + Social Proof) ===
...
- Tone: casual, like you're DMing someone you met at a conference`
    ...
    taskPrompt = `=== TASK: BREAK-UP (Day 21+, The Standing Offer) ===
...
- They should feel like they're losing access to something valuable, not being pestered.`
  ...
  }>(FOLLOWUP_SYSTEM_PROMPT, context + '\n\n' + taskPrompt, {
    temperature: 0.95,
    maxTokens: 4096,
  })

  const body = stripEmDashes(result.body)
  const subject = stripEmDashes(result.subject)

  return {
    subject,
    body,
    wordCount: countWords(body),
    quality: checkEmailQuality(subject, body, 'follow_up'),
    channel: result.channel,
    ctaType: 'mckenna', // default for follows
  }
}
