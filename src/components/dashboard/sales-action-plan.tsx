...
const CATEGORY_ICONS: Record<string, React.ReactNode> = {
  ...
  review: <ClipboardCheck className="h-4 w-4 text-indigo-500" />,
  prep: <Swords className="h-4 w-4 text-orange-500" />,
};

const PRIORITY_CONFIG = {
  critical: { label: 'Critical', color: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400', dot: 'bg-red-500' },
  high: { label: 'High', color: 'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400', dot: 'bg-amber-500' },
  medium: { label: 'Medium', color: 'bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400', dot: 'bg-blue-500' },
};
    ...
    <div className="rounded-xl border border-zinc-200/80 dark:border-zinc-700/80 bg-white dark:bg-zinc-900/50 p-5">
      ...
      <div className="space-y-3">
        {actions.map((action) => {
          ...
          const canMagicDraft = onMagicDraft && ['prospecting', 'follow_up'].includes(action.category);
          const canResearch = onResearch && action.category === 'research' && action.id.endsWith(':research');
          const canPrep = onPrep && action.category === 'prep';
          const canMemo = onInvestorMemo && action.category === 'research' && action.id.endsWith(':memo');
              ...
              <div className="flex items-start gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <div className="flex items-center gap-1.5" title={`${priority.label} priority`}>
                      <div className={`h-1.5 w-1.5 rounded-full ${priority.dot}`} />
                      <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-tight ${priority.color}`}>
                        {priority.label}
                      </span>
                    </div>
                    <Link
                  ...
                  <div className="group/action relative flex items-start gap-2 bg-white dark:bg-zinc-900/40 p-2.5 rounded-lg border border-zinc-100 dark:border-zinc-800">
                    ...
                    <CopyButton
                      ...
                      label="Action"
                      className="absolute top-1.5 right-1.5 opacity-0 group-hover/action:opacity-100 focus:opacity-100"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                {canMagicDraft && (
                  <button
                    ...
                    aria-label={isLeadProcessing ? "Drafting next step" : "Magic Draft"}
                  >
                    {isLeadProcessing ? (
                ...
                {canPrep && (
                  <button
                    ...
                    disabled={!!isProcessing}
                    title="Run AI Prep"
                    className="flex items-center gap-1.5 px-3 py-2 text-[11px] font-bold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/30 hover:bg-orange-100 dark:hover:bg-orange-900/50 rounded-lg transition-colors border border-orange-100 dark:border-orange-800 disabled:opacity-50"
                  >
                    {isLeadProcessing ? (
                      <Loader2 className="h-3.5 w-3.5 animate-spin" />
                    ) : (
                ...
                {canMemo && (
                  ...
                  >
                    ...
                    {isLeadProcessing ? 'Generating...' : 'Memo'}
                  </button>
                )}
