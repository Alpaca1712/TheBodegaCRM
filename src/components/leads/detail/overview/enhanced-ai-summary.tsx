...
import { Brain, CheckCircle2, ArrowRight, Zap, Linkedin, Twitter, Phone, MapPin, Hash, Loader2 } from 'lucide-react';
...
export function EnhancedAISummary({ lead, onRefresh }: EnhancedAISummaryProps) {
  ...
  return (
    <div className="rounded-xl border border-zinc-200/80 dark:border-zinc-700/80 bg-white dark:bg-zinc-900/50 p-5 space-y-3">
      ...
      {parsed && (
        <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 space-y-3 relative group">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
      ...
      {!parsed && lead.conversation_next_step && (
        <div className="flex items-center justify-between p-4 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 group">
          ...
          <button
            onClick={handleMagicDraft}
            disabled={isDrafting}
            className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-white bg-amber-500 hover:bg-amber-600 rounded-lg transition-colors shadow-sm shadow-amber-500/20 disabled:opacity-50 ml-4 shrink-0"
          >
            {isDrafting ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Zap className="h-3.5 w-3.5 fill-current" />}
            Magic Draft
          </button>
