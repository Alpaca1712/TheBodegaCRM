...
const shortcutGroups = [
  {
    ...
    shortcuts: [
      ...
      { keys: ['G', 'C'], description: 'Open Co-pilot' },
    ],
  },
  {
    label: 'Quick Actions',
...
export default function KeyboardShortcutsDialog() {
  ...
  useEffect(() => {
    ...
    document.addEventListener('toggle-keyboard-shortcuts', handler)
    // Also support the old event just in case
    document.addEventListener('show-keyboard-shortcuts', handler)
    return () => {
      document.removeEventListener('toggle-keyboard-shortcuts', handler)
      document.removeEventListener('show-keyboard-shortcuts', handler)
    }
  }, [])
  ...
  useEffect(() => {
    if (open) {
      ...
      // Small timeout to allow the dialog to render before focusing
      ...
      <div className="relative bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-xl w-full max-w-md overflow-hidden animate-scale-in">
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100 dark:border-zinc-800">
          ...
          <button
            ...
            className="p-1 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/50"
