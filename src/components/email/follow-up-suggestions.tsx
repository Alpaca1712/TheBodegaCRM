...
interface FollowUpItem {
  ...
  suggestedType: 'follow_up_1' | 'follow_up_2' | 'follow_up_3' | 'break_up' | 'reply_needed' | 'post_meeting' | 'initial_outreach' | 'review_draft';
...
const ACTION_LABELS: Record<string, { label: string; short: string; description: string }> = {
  initial_outreach: { label: 'Initial SMYKM', short: 'First Send', description: 'Deep research complete. Draft the first SMYKM email.' },
  review_draft: { label: 'Review & Send', short: 'Send Draft', description: 'Draft is ready in CRM. Move it to Gmail and send.' },
...
function computeFollowUp(lead: Lead, allEmails: LeadEmail[]): FollowUpItem | null {
  ...
  if (lead.stage === 'researched') {
    ...
    return {
      ...
      urgency: daysSinceCreated >= 1 ? 'due_today' : 'upcoming',
      sequenceDay: 'Day 0', outboundCount, inboundCount,
    };
  }
  ...
  if (lead.stage === 'email_drafted') {
    ...
    return {
      ...
      urgency: 'due_today',
      sequenceDay: 'Draft', outboundCount, inboundCount,
    };
  }
